	...
	int pid)
{
	...
	if (g_wifi_diag_log == 1) {
		DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "diag: In %s, hdlc_disabled:%02x\n", __func__, hdlc_disabled);
	}
...
int diag_process_apps_pkt(unsigned char *buf, int len, int pid)
{
	...
	if (g_wifi_diag_log == 1) {
		DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "CMD:%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x, len:%d\n",
			*(buf), *(buf+1), *(buf+2), *(buf+3), *(buf+4), *(buf+5), *(buf+6), *(buf+7), *(buf+8), *(buf+9), *(buf+10),
			 *(buf+11), *(buf+12), *(buf+13), *(buf+14), *(buf+15), len);
	}
	...
	if (write_len > 0) {
		...
		if (info) {
			...
			if (g_wifi_diag_log == 1)
				DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "diag: In %s, session disabling HDLC encoding\n", __func__);
		} else {
			driver->hdlc_disabled = 1;
			if (g_wifi_diag_log == 1)
				DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "diag: In %s, driver disabling HDLC encoding\n", __func__);
		}
		peripheral =
...
void diag_process_non_hdlc_pkt(unsigned char *buf, int len, int pid)
{
	...
	if (partial_pkt->remaining == 0) {
		...
		if (*(uint8_t *)(data_ptr + actual_pkt->length) !=
			...
			mutex_unlock(&driver->hdlc_recovery_mutex);
			if (g_wifi_diag_log == 1)
				printk("diag: diag_hdlc_start_recovery:%d %s\n", __LINE__, __FUNCTION__);
...
static int diagfwd_mux_read_done(unsigned char *buf, int len, int ctxt)
{
	...
	if (g_wifi_diag_log == 1) {
		DIAG_LOG(DIAG_DEBUG_PERIPHERALS, "CMD:%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x, hdlc_disabled:%02x\n",
			...
			*(buf+10), *(buf+11), driver->hdlc_disabled);
	}
