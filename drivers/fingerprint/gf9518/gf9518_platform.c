...
int gf9518_parse_dts(struct gf_dev *gf_dev)
{
	...
	gf_dev->led_use_gpio = of_property_read_bool(gf_dev->spi->dev.of_node, "goodix,led_use_gpio");
	gf_dev->led_use_pmic = of_property_read_bool(gf_dev->spi->dev.of_node, "goodix,led_use_pmic");
	...
	pr_info("%s led_use_gpio %d\n", __func__, gf_dev->led_use_gpio);
	pr_info("%s led_use_pmic %d\n", __func__, gf_dev->led_use_pmic);
	...
	// LED
	if (gf_dev->led_use_gpio) {
		gf_dev->led_en_gpio = of_get_named_gpio(gf_dev->spi->dev.of_node, "goodix,gpio_led_en", 0);
		if (!gpio_is_valid(gf_dev->led_en_gpio)) {
			pr_info("%s LED_EN GPIO is invalid.\n", __func__);
			return -EIO;
		} else {
			pr_info("%s LED_EN GPIO is %d.\n", __func__, gf_dev->led_en_gpio);
		}
		rc = gpio_request(gf_dev->led_en_gpio, "goodix_led_en");
		if (rc) {
			pr_info("%s Failed to request LED_EN GPIO. rc = %d\n", __func__, rc);
			return -EIO;
		} else {
			pr_info("%s Success to request LED_EN GPIO. rc = %d\n", __func__, rc);
		}
	}

	if (gf_dev->led_use_pmic) {
		gf_dev->led = regulator_get(&gf_dev->spi->dev, "led");
		if (IS_ERR(gf_dev->led)) {
		pr_info("%s Unable to get led\n", __func__);
			return -EINVAL;
		} else {
			pr_info("%s Success to get led\n", __func__);
		}
	}
...
int gf9518_power_on(struct gf_dev *gf_dev)
{
	...
	// LED ON
	if (gf_dev->led_use_gpio) {
		rc = gpio_direction_output(gf_dev->led_en_gpio, 1);
		if (rc) {
			pr_info("gf9518 led power on fail.\n");
			return -EIO;
		}
	}

	if (gf_dev->led_use_pmic) {
		if (regulator_is_enabled(gf_dev->led)) {
			pr_info("%s, led state:on,don't set repeatedly!\n", __func__);
			return rc;
		}

		rc = regulator_set_load(gf_dev->led, 600000);
		if (rc < 0) {
			pr_err("%s: led regulator_set_load(uA_load=%d) failed. rc=%d\n",
			__func__, 1000, rc);
		}

		if (regulator_count_voltages(gf_dev->led) > 0) {
			rc = regulator_set_voltage(gf_dev->led, 3300000, 3300000);
			if (rc) {
				pr_info(KERN_ERR "gf9518:Unable to set voltage on led");
			}
		}
		rc = regulator_enable(gf_dev->led);
	}
...
int gf9518_power_off(struct gf_dev *gf_dev)
{
	...
	// LED OFF
		if (gf_dev->led_use_gpio) {
			rc = gpio_direction_output(gf_dev->led_en_gpio, 0);
			if (rc) {
			pr_info("gf9518 vddio power off fail.\n");
			return -EIO;
			}
		...
		if (gf_dev->led_use_pmic) {
		rc = regulator_set_load(gf_dev->led, 0);
			if (rc < 0) {
				pr_err("%s: led regulator_set_load(uA_load=%d) failed. rc=%d\n",
					__func__, 0, rc);
			}
			if (regulator_is_enabled(gf_dev->led)) {
				regulator_disable(gf_dev->led);
			}
			pr_info(KERN_ERR "gf9518: disable  led %d\n", rc);
		}
