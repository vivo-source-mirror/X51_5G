...
//	wake_lock_init(&vivo_wakeup_t.fp_wakelock, WAKE_LOCK_SUSPEND, "fp_wakelock");
	...
	gf_dev->led_use_gpio = false;
	gf_dev->led_use_pmic = false;
	...
	if (status == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(keys_maps); i++)
			input_set_capability(gf_dev->input, keys_maps[i].type, keys_maps[i].code);
