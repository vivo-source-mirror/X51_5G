...
struct geni_i2c_dev {
	...
	int err;
	u32 xfer_timeout;
	u32 tp_xfer_timeout; /*add for touchscreen team*/
...
{
	struct geni_i2c_dev *gi2c = i2c_get_adapdata(adap);
	int i, ret = 0, timeout = 0, xfer_timeout = HZ;
	...
	for (i = 0; i < num; i++) {
		...
		mb();
		if (gi2c->tp_xfer_timeout)
			gi2c->xfer_timeout = gi2c->tp_xfer_timeout;
		timeout = wait_for_completion_timeout(&gi2c->xfer,
						gi2c->xfer_timeout);
		if (!timeout) {
			GENI_SE_ERR(gi2c->ipcl, true, gi2c->dev,
		...
		if (gi2c->err) {
			...
			if (gi2c->tp_xfer_timeout)
				xfer_timeout = gi2c->tp_xfer_timeout;
			timeout = wait_for_completion_timeout(&gi2c->xfer, xfer_timeout);
			if (!timeout) {
				GENI_SE_ERR(gi2c->ipcl, true, gi2c->dev,
					"Abort\n");
...
static int geni_i2c_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "qcom,tp-xfer-timeout",
				&gi2c->tp_xfer_timeout))
		gi2c->tp_xfer_timeout = 0;
	else
		dev_info(&pdev->dev,
			"VIVO_TS tp xfer timeout specified %d.\n", gi2c->tp_xfer_timeout);
