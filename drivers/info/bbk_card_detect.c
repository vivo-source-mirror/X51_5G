...
struct card_detect_data {
	...
	unsigned int sim1_gpio_value;
	unsigned int sim2_gpio_value;
	unsigned int tf_card_gpio_value;

	unsigned int sim1_gpio;
	unsigned int sim2_gpio;
	unsigned int tf_card_gpio;
	unsigned int sim1_gpio_flags;
	unsigned int sim2_gpio_flags;
	unsigned int tf_card_gpio_flags;
};
...
static ssize_t card_slot_inserted_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int gpio_state = -1;
	int count = 0;
	int flag = 0;
...
static ssize_t card_slot_removed_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int gpio_state = -1;
	int count = 0;
	int flag = 0;
...
static ssize_t card_slot_status_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (cd_data->sim1_card) {
		...
		if (gpio_state < 0) {
			...
			count += snprintf(&buf[count], PAGE_SIZE, "GPIO%d-SIM1:1,", cd_data->sim1_gpio);
		} else if (gpio_state != cd_data->sim1_gpio_value) {  /*open drain */
			count += snprintf(&buf[count], PAGE_SIZE, "GPIO%d-SIM1:0,", cd_data->sim1_gpio);
		}
	}
	...
	if (cd_data->sim2_card) {
		...
		if (gpio_state < 0) {
			...
			count += snprintf(&buf[count], PAGE_SIZE, "GPIO%d-SIM2:1,", cd_data->sim2_gpio);
		} else if (gpio_state != cd_data->sim2_gpio_value) {
			count += snprintf(&buf[count], PAGE_SIZE, "GPIO%d-SIM2:0,", cd_data->sim2_gpio);
		}
	}
	...
	if (cd_data->tf_card) {
		...
		if (gpio_state < 0) {
			...
			count += snprintf(&buf[count], PAGE_SIZE, "GPIO%d-TF:1", cd_data->tf_card_gpio);
		} else if (gpio_state != cd_data->tf_card_gpio_value) {
			count += snprintf(&buf[count], PAGE_SIZE, "GPIO%d-TF:0", cd_data->tf_card_gpio);
		}
	}
