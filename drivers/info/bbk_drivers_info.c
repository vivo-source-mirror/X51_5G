...
int bbk_driver_register_device_info(char *device_type, char *device_name)
{
	...
	strlcpy(new_info->dev_info.device_type, device_type, 255);
	strlcpy(new_info->dev_info.device_name, device_name, 255);
...
int get_bbk_hw_subtype(void)
{
	...
	if (of_property_read_u32_array(root, "qcom,board-id", subtype, 2) != 0) {
		printk(KERN_ERR " Can't find prop qcom,board-id \n");
		return -ENODEV;
	}
...
/*zg add for PD1806
return 1 mean UD finger
return 0 mean ca finger*/
int get_bbk_hardware_Id(void)
{
	if (bbk_board_version[3] == '1') {
		return 1;
	} else if (bbk_board_version[3] == '0') {
		return 0;
	}
	return 1;
}
EXPORT_SYMBOL_GPL(get_bbk_hardware_Id);

static __init int set_bbk_board_version(char *str)
{
	strlcpy(bbk_board_version, str, 255);
...
{
	strlcpy(bbk_model_version, str, 255);
...
{
	strlcpy(mpp_dig_val, str, 255);
	printk(KERN_ERR "mpp digital is %s\n", mpp_dig_val);
	return 0;
}
							...
							struct kobj_attribute *attr, char *buf)
{
	...
	if (!list_empty(&devices_list.list)) {
		/*count += snprintf(&buf[count], 1023, "%-16.s%-16.s\n", "Device type", "Device name"); */
		list_for_each_entry(entry, &devices_list.list, list) {
			count += snprintf(&buf[count], 1023, "A = ");
			count += snprintf(&buf[count], 1023, "%s\n", entry->dev_info.device_type);
			/*count += snprintf(&buf[count], 1023, "%-16.s\n", entry->dev_info.device_name); */
		}
	} else {
		count += snprintf(buf, 1023, "Device list is empty\n");
	}
