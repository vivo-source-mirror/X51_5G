...
#ifdef CONFIG_VIVO_LINUXBSP_EDIT
char qpnp_pon_reason_str[64];
char qpnp_poff_reason_str[64];
					    ...
					    struct device_node *node)
{
	...
	if (is_debug_mode &&
		...
		/*Support KPDPWR_RESIN combo dump*/
		cfg->s2_type = 1;
		cfg->s1_timer = 6720;
		cfg->s2_timer = 500;
		pr_err("s2_type:%d is_debug_mode:%d\n",
				cfg->s2_type, is_debug_mode);
	}
#endif
...
#ifndef CONFIG_VIVO_LINUXBSP_EDIT
...
#ifdef CONFIG_VIVO_LINUXBSP_EDIT
//vivo hankecai add for pon_init_customization begin
...
#endif	/*CONFIG_VIVO_WDT_MONITOR*/
//vivo hankecai add for pon_init_customization end
...
static int qpnp_pon_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_volume_down_wake);
	cancel_delayed_work_sync(&pon->kpdpwr_work);
#endif	/*CONFIG_VIVO_LINUXBSP_EDIT*/
