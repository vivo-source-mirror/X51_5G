...
/*#define vts_dev_log(force, flag, fmt, param...) do { \
	if(likely(force)) \
		printk(KERN_ERR "VIVO_TS_"flag"[%s]%s:%d:"fmt, __func__, __LINE__, ##param); \
} while (0)
#define vts_dev_err(fmt, param...) do { \
	vts_dev_log(1, "ERR", fmt, ##param); \
} while(0)
		
#define vts_dev_info(fmt, param...) vts_dev_log(1, "INF", fmt, ##param)
#define vts_dev_dbg(fmt, param...) vts_dev_log(0, "DBG", fmt, ##param)
...
/*#define VTI(fmt, param...) printk(KERN_ERR "VIVO_TS_INF[%s]%s:%d:"fmt, __func__, __LINE__, ##param)
#define VTE(fmt, param...) printk(KERN_ERR "VIVO_TS_ERR[%s]%s:%d:"fmt, __func__, __LINE__, ##param)
#define VTD(fmt, param...) do {} while(0)*/
