...
#ifdef CONFIG_VIVO_CUSTOM_MINIDUMP
/* when add minidump, shoud not provide this interface */
#else
...
#endif	/*CONFIG_VIVO_CUSTOM_MINIDUMP*/
...
#ifdef CONFIG_VIVO_LINUXBSP_EDIT
	if (dload_mode_enabled)
		qpnp_pon_system_pwr_off(PON_POWER_OFF_WARM_RESET);
	else
		qpnp_pon_system_pwr_off(PON_POWER_OFF_HARD_RESET);

	is_debug_mode = dload_mode_enabled;
	qpnp_pon_kpdpwr_resin_ex(dload_mode_enabled);
#endif
#endif//CONFIG_VIVO_CUSTOM_MINIDUMP
}
...
{
	pr_err("reboot edl is not support on target\n");
}
#else
static void enable_emergency_dload_mode(void)
{
...
static void msm_restart_prepare(const char *cmd)
{
	...
	printk(KERN_ERR "%s %d need_warm_reset = %d, get_dload_mode() = %d, cmd = %s\n", 
						__func__, __LINE__, need_warm_reset, get_dload_mode(), cmd);
#endif
