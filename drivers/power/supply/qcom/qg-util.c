...
/*vivo add*/
static void qg_data_process_for_batt_temp(struct qpnp_qg *chip, int *batt_temp)
{
	static int pre_batt_temp = 0;
	static bool init_flag = false;
	static bool abnor_flag = false;

	mutex_lock(&chip->dp_batt_temp_lock);
	if (!init_flag && chip->qg_monitor_work_count < 1) {
		pr_err("init dp batt_temp. not ready.\n");
		goto out;
	}
	...
	if (!init_flag) {
		...
		init_flag = true;
		pr_err("init dp batt_temp.\n");
		goto out;
	}
	//pr_err("init dp :%d, now:%d abs=%d \n", pre_batt_temp, *batt_temp, abs(*batt_temp - pre_batt_temp));

	if (abs(*batt_temp - pre_batt_temp) >= 100) {
		if (!abnor_flag) {
			pr_err("abnormal temp,use pre_batt_temp:%d, now:%d\n", pre_batt_temp, *batt_temp);
			abnor_flag = true;
...
out:
	mutex_unlock(&chip->dp_batt_temp_lock);
}
/*vivo end*/
...
int qg_get_battery_temp(struct qpnp_qg *chip, int *temp)
{
	...
	/*vivo add*/
	qg_data_process_for_batt_temp(chip, temp);
	/*vivo end*/
