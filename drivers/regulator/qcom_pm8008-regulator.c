...
static int pm8008_reset(void)
{
	...
	mutex_lock(&rst_data.lock);
	if (rst_data.pm8008_tpvdd != NULL) {
		rc = regulator_force_disable(rst_data.pm8008_tpvdd);
		mdelay(6);
		if (rc)
			pr_err("failed to force disable tpvdd\n");
		else
			pr_info("success to force disable tpvdd\n");
	}
...
static int pm8008_chip_probe(struct platform_device *pdev)
{
	...
	rst_data.pm8008_tpvdd = regulator_get(chip->dev, "qcom,pm8008_tpvdd");
	if (gpio_is_valid(rst_data.reset_gpio)) {
