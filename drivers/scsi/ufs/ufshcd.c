...
int ufshcd_hold(struct ufs_hba *hba, bool async)
{
	...
	if ((hba->clk_gating.state == CLKS_ON) &&
		...
		pr_err_ratelimited("%s: %s \n",
			async ? "host blocked" : "may deadloop", __func__);
	}
...
static int ufshcd_complete_dev_init(struct ufs_hba *hba)
{
	...
	if (err)
		...
		dev_err(hba->dev,
			...
			__func__);
		panic("%s fDeviceInit was not cleared by the device\n",
			__func__);
	}
...
static void ufshcd_rls_handler(struct work_struct *work)
{
	...
	if (!down_write_trylock(&hba->lock)) {
		queue_delayed_work(hba->recovery_wq, &hba->rls_work, msecs_to_jiffies(500));
		pm_runtime_put_sync(hba->dev);
		return;
	}
	ufshcd_scsi_block_requests(hba);
	...
	if ((reg & UIC_PHY_ADAPTER_LAYER_ERROR) &&
		...
		if (reg & UIC_PHY_ADAPTER_LAYER_GENERIC_ERROR) {
			...
			if (!hba->full_init_linereset)
				queue_delayed_work(hba->recovery_wq, &hba->rls_work, 0);
		}
		retval |= IRQ_HANDLED;
	}
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	hba->recovery_wq = alloc_workqueue("%s", 
		WQ_MEM_RECLAIM|WQ_UNBOUND|WQ_HIGHPRI, 0, recovery_wq_name);
	if (!hba->recovery_wq) {
