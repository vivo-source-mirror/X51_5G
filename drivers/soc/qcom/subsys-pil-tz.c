...
#define CDS_MODULE_ID	"cds_3300"
#define MODULE_ID		"3300"
...
static void subsys_crash_collect(struct work_struct *work)
{
	...
	int len = 0;
	int i = 0;
	char subtype_name[][10] = {"modem", "adsp", "cdsp", "esoc0", "others"};
	...
	/* [{"moduleid":"3300","eventId":"00059|012","extype":"1","subtype":"1_2_3_4_5","times":"1","exInfo": "name:reason"}] */
	...
	scnprintf(pos, 200, "%d\",\"times\":\"1\",\"exInfo\":\"%s:%s\"}]", subtype_id, subtype, crash_reason);
...
/* not GDPR code start */
#ifdef CONFIG_FMEA
	writeData(CDS_MODULE_ID, NULL, ex_data);
	if (2 == subtype_id && !strnstr(crash_reason, "adsp_stress_test", sizeof("adsp_stress_test")))
		writeDatas("1800", "1800_1", "adsp subsystem failure reason: %s.\n", crash_reason);
#endif
#ifdef CONFIG_RSC_VAUDIT
	if (2 == subtype_id && !strnstr(crash_reason, "adsp_stress_test", sizeof("adsp_stress_test")))
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_ADSP_SUBSYSTEM, 0,
				"adsp subsystem failure reason: %s.", crash_reason);
#endif
/* not GDPR code end */
#endif
...
static void log_failure_reason(const struct pil_tz_data *d)
{
	...
	if (d->smem_id == -1) {
		strlcpy(crash_reason, "pil_tz_data smem_id equals '-1'", min(size, (size_t)(MAX_SSR_REASON_LEN - 1)));
...
{
	/* vivo marongqiang add for subsystem crash collect start */
	pr_info("%s subsystem failure collect.\n", __func__);
