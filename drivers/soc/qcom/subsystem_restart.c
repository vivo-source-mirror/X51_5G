...
extern unsigned int is_fastboot_debug_mode;
...
int subsystem_restart_dev(struct subsys_device *dev)
{
	...
	/* Temporarily avoid subsystem crash in BSP pressure test, chenming. */
	pr_err("Fastboot debug is_debug_mode=%d, is_fastboot_debug_mode=%d\n", is_debug_mode, is_fastboot_debug_mode);
	if ((1 == is_debug_mode) && (0 == is_fastboot_debug_mode)) {
		dev->restart_level = RESET_SUBSYS_COUPLED;
		pr_err("Fastboot debug restart sequence requested for %s, restart_level = %s.\n",
			name, restart_levels[dev->restart_level]);
	}
...
#ifdef CONFIG_VIVO_LINUXBSP_EDIT
		subsys->restart_level = RESET_SUBSYS_COUPLED;
		if (is_debug_mode && !force_ssr_related)
					subsys->restart_level = RESET_SOC;

		printk("subsys[%s] restart_level[%s]\n", desc->name,
