...
#define CHECK_OTG_DISABLE_WORK_PEROID_MS        1000
...
#define MAX_GSI_EP_IN_BUF_SIZE	(1589248 + 2048)
...
#define MDWC3_POWER_COLLAPSE		BIT(2)
	...
	void		*mdwc_buf_base_addr;
	dma_addr_t mdwc_buf_dma;
	size_t mdwc_buf_len;
	...
	bool usb_detc_used;
    bool otg_sf_switch;
	struct wakeup_source otg_switch_lock;
	struct delayed_work otg_disabled_work;
	int retry_times;
    /*vivo add end*/
};
			...
			struct usb_gsi_request *request)
{
	...
	dev_dbg(mdwc->dev, "GSI DB Addr %d: %llx\n", n,
		(unsigned long long)request->mapped_db_reg_phs_addr_lsb);
}
...
static int gsi_prepare_trbs(struct usb_ep *ep, struct usb_gsi_request *req)
{
	...
	/* IN direction */
	if (dep->direction) {
		if ((mdwc->mdwc_buf_base_addr) && (len < mdwc->mdwc_buf_len)) {
			req->buf_base_addr = mdwc->mdwc_buf_base_addr;
			req->dma = mdwc->mdwc_buf_dma;
		} else {
			dev_err(dwc->dev, "mdwc_buf_dma not exist %s, len have 0x%x, req 0x%x\n",
				dep->name, mdwc->mdwc_buf_len, len);
			return -ENOMEM;			
		}
	/*OUT dir*/
	} else {
...
static void disabled_otg_work(struct dwc3_msm *motg)
{
	int vote_val = 0;

	motg->otg_sf_switch = false;
	motg->retry_times = 0;

	vote_val = get_client_vote_locked(motg->drp_mode_votable, "OTG_DRP_VOTER");
	if (vote_val == motg->otg_sf_switch) {
		pr_info("usb_detc_used: already set drp mode votable to %d\n", vote_val);
		goto done;
	}
	vote(motg->drp_mode_votable, "OTG_DRP_VOTER", motg->otg_sf_switch, 1);

	__pm_relax(&motg->otg_switch_lock);
done:
	pr_info("%s: check otg switch timeout, set power role to SINK.\n", __func__);
}

static void check_otg_disabled_work(struct work_struct *work)
{
	union power_supply_propval pval = {0,};
	struct dwc3_msm *motg = container_of(work, struct dwc3_msm, otg_disabled_work.work);
	...
	//pr_info("%s: out current typec mode: %d. retry time: %d\n", __func__, pval.intval, motg->retry_times);

	if (motg->retry_times <= 0) {
		pr_info("%s: current typec mode: %d. retry time: %d\n", __func__, pval.intval, motg->retry_times);
		disabled_otg_work(motg);
		return;
	} else if (pval.intval == POWER_SUPPLY_TYPEC_SINK || pval.intval == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER
		|| pval.intval == POWER_SUPPLY_TYPEC_SINK_POWERED_CABLE || pval.intval == POWER_SUPPLY_TYPEC_POWERED_CABLE_ONLY
		|| pval.intval == POWER_SUPPLY_TYPEC_SINK_DEBUG_ACCESSORY) {
		motg->retry_times = 300;
		goto retry;
	}

	motg->retry_times--;
retry:
	schedule_delayed_work(&motg->otg_disabled_work, msecs_to_jiffies(CHECK_OTG_DISABLE_WORK_PEROID_MS));
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	case DRD_STATE_PERIPHERAL:
		...
		if (motg->usb_detc_used && pval.intval == USB_DET_PIN_MODE_DET)
		...
		struct device_attribute *attr, char *buf)
{
	...
	if ((!motg->usb_detc_used && pval.intval == 0) || 
		(motg->usb_detc_used && motg->otg_sf_switch)) {
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	char value[32];
	struct dwc3_msm *motg;
	union power_supply_propval gpval = {0,};
	union power_supply_propval spval = {0,};
	
	pr_info("add otg_host_mode_enable_store start! \n");
    motg = dev_get_drvdata(dev);
	if (!motg) {
		return -ENODEV;
	}
	...
	if (motg->usb_detc_used && !motg->drp_mode_votable) {
		motg->drp_mode_votable = find_votable("DRP_MODE");
		if (!motg->drp_mode_votable) {
			pr_info("Could not get usb drp mode votable\n");
			return -EINVAL;
		}
	}
	if (sscanf(buf, "%s", value) != 1) {
	...
	if (!strncmp(value, "enabled", 6)) {
		...
		if (motg->usb_detc_used) {
			if (motg->otg_sf_switch) {
				pr_info("otg switch alrady enabled, ignor.\n");
				goto done;
			}
			motg->retry_times = 300;
			motg->otg_sf_switch = true;
			__pm_stay_awake(&motg->otg_switch_lock);
			schedule_delayed_work(&motg->otg_disabled_work, 0);
		} else
			spval.intval = 0x01;
		...
		if (motg->usb_detc_used) {
			if (!motg->otg_sf_switch) {
				pr_info("otg switch alrady disabled, ignor.\n");
				goto done;
			}
			motg->retry_times = 0;
			motg->otg_sf_switch = false;
			__pm_relax(&motg->otg_switch_lock);
			cancel_delayed_work(&motg->otg_disabled_work);
		} else
			spval.intval = 0x0;
	} else {
		return -EINVAL;
	...
	if (motg->usb_detc_used) {
		...
		vote_val = get_client_vote_locked(motg->drp_mode_votable, "OTG_DRP_VOTER");
		if (vote_val == motg->otg_sf_switch) {
			pr_info("usb_detc_used: true, already set drp mode votable to %d\n", vote_val);
			goto done;
		}
		vote(motg->drp_mode_votable, "OTG_DRP_VOTER", motg->otg_sf_switch, 1);
		pr_info("usb_detc_used: set drp mode votable %d\n", motg->otg_sf_switch);
	} else {
		power_supply_get_property(motg->usb_psy,
				POWER_SUPPLY_PROP_TYPEC_SWITCH, &gpval);
		if (gpval.intval != spval.intval) {
			pr_info("otg_sf_switch: true, already set typec switch to %d\n", !gpval.intval);
			goto done;
		}
		
		power_supply_set_property(motg->usb_psy,
				POWER_SUPPLY_PROP_TYPEC_SWITCH, &spval);
	}
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	mdwc->usb_detc_used = of_property_read_bool(node,
				"vivo,usbc_det_used");
	if (mdwc->usb_detc_used)
		dev_dbg(&pdev->dev, "current use usbc_det irq\n");
...
//hguan add begin
    mdwc->mdwc_buf_len = MAX_GSI_EP_IN_BUF_SIZE;
    if (mdwc->gsi_io_coherency_disabled)
		dma_attr = DMA_ATTR_FORCE_NON_COHERENT;
	else
		dma_attr = DMA_ATTR_FORCE_COHERENT;
	...
	/* Allocate TRB buffers */
	mdwc->mdwc_buf_base_addr = dma_alloc_attrs(dwc->sysdev, mdwc->mdwc_buf_len, &mdwc->mdwc_buf_dma,
					GFP_KERNEL, dma_attr);
	if (!mdwc->mdwc_buf_base_addr) {
		dev_err(dwc->dev, "mdwc_buf_base_addr probe allocate failed, len 0x%x.\n", mdwc->mdwc_buf_len);
		mdwc->mdwc_buf_len = 0;
		goto put_dwc3;
	}		
//hguan add end
	...
	/* vivo add start */
	INIT_DELAYED_WORK(&mdwc->otg_disabled_work, check_otg_disabled_work);
	wakeup_source_init(&mdwc->otg_switch_lock, "switch_wakelock");
	...
	if (device_create_file(&pdev->dev, &dev_attr_usb_type)) {
		pr_err("Failed to create usb_type file");
    }
	/* vivo add end */
...
put_dwc3:
	if (mdwc->mdwc_buf_base_addr) {
		dma_free_attrs(dwc->sysdev, MAX_GSI_EP_IN_BUF_SIZE,
		    mdwc->mdwc_buf_base_addr, mdwc->mdwc_buf_dma, dma_attr);
		mdwc->mdwc_buf_len = 0;
	    mdwc->mdwc_buf_base_addr = NULL;
	}	
