...
#define USB_HSPHY_1P8_VOL_MIN			1890000 /* uV */
#define USB_HSPHY_1P8_VOL_MAX			1890000 /* uV */
#define USB_HSPHY_1P8_HPM_LOAD			19000	/* uA */

#ifdef CUSTOMIZE_L12A_180
#undef USB_HSPHY_1P8_VOL_MIN
#undef USB_HSPHY_1P8_VOL_MAX
#define USB_HSPHY_1P8_VOL_MIN			1800000 /* uV */
#define USB_HSPHY_1P8_VOL_MAX			1800000 /* uV */
#endif	/*CUSTOMIZE_L12A_180*/
...
static int msm_hsphy_probe(struct platform_device *pdev)
{
	...
	if (board_version) {
		dev_dbg(dev, "%s: %d: board_version = %s\n", __func__, __LINE__, board_version);
		if (board_version[4] == '0') {
			phy->param_override_seq_cnt = of_property_count_elems_of_size(
							dev->of_node,
							"qcom,param-override-seq-board1",
			...
			if (phy->param_override_seq_cnt > 0) {
				...
				ret = of_property_read_u32_array(dev->of_node,
						...
						phy->param_override_seq_cnt);
				if (ret) {
					dev_err(dev, "qcom,param-override-seq-board1 read failed %d\n",
						ret);
					return ret;
				}
			}
			...
			phy->param_override_seq_cnt_host = of_property_count_elems_of_size(
							dev->of_node,
							"qcom,param-override-seq-board1-host",
			...
			if (phy->param_override_seq_cnt_host > 0) {
				...
				ret = of_property_read_u32_array(dev->of_node,
						...
						phy->param_override_seq_cnt_host);
				if (ret) {
					dev_err(dev, "qcom,param-override-seq-board1-host read failed %d\n",
						ret);
					return ret;
				}
			}
		...
		dev_err(dev, "%s: %d: failed to get board_version", __func__, __LINE__);
