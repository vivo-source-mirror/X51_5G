...
//vivo chenming add for subsystem begin
unsigned int is_fastboot_debug_mode;
EXPORT_SYMBOL(is_fastboot_debug_mode);
module_param(is_fastboot_debug_mode, int, 0444);
//vivo chenming add for subsystem end
...
//vivo linchunsheng add for minidump start
extern int download_mode;
//vivo linchunsheng add for minidump end
...
static void __init is_at_boot(char *command_line)
{
	...
	char *is_download_mode = "msm_poweroff.download_mode=1";
	...
	if (strnstr(command_line, is_download_mode, strlen(command_line)) == NULL) {
		is_fastboot_debug_mode = 0;
	} else {
		is_fastboot_debug_mode = 1;
	}
	...
	if (strnstr(command_line, "sendNormal", strlen(command_line)) || strnstr(command_line, "sendAT_first", strlen(command_line))
		|| strnstr(command_line, "init.at.rc", strlen(command_line)) || strnstr(command_line, "init.at.dl.rc", strlen(command_line))) {
		is_atCmd = 1;
	} else {
		is_atCmd = 0;
	}
	...
	//vivo hankecai add for normal_boot_mode begin
	if ((power_off_charging_mode == 1) || (is_atboot == 1))
		...
		is_normal_mode = 0;
	//vivo hankecai add for normal_boot_mode end
}
/*AT modem end*/
#ifdef CONFIG_VIVO_LINUXBSP_EDIT
